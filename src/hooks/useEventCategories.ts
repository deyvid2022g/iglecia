...
import { eventService, type EventCategory, type EventCategoryInsert, type EventCategoryUpdate } from '../services/eventService';

interface UseEventCategoriesOptions {
  realtime?: boolean;
}

export const useEventCategories = (options: UseEventCategoriesOptions = {}) => {
  ...
  const fetchCategories = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await eventService.categories.getAll();
      setCategories(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Error fetching event categories');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createCategory = useCallback(async (category: EventCategoryInsert) => {
    try {
      setError(null);
      const newCategory = await eventService.categories.create(category);
  ...
  const updateCategory = useCallback(async (id: string, updates: EventCategoryUpdate) => {
    try {
      setError(null);
      const updatedCategory = await eventService.categories.update(id, updates);
      setCategories(prev => 
  ...
  const deleteCategory = useCallback(async (id: string) => {
    try {
      setError(null);
      await eventService.categories.delete(id);
  ...
  const getCategoryById = useCallback(async (id: string) => {
    try {
      setError(null);
      return await eventService.categories.getById(id);
    } catch (err) {
  ...
  const getCategoryBySlug = useCallback(async (slug: string) => {
    try {
      setError(null);
      return await eventService.categories.getBySlug(slug);
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Error fetching category';
      setError(errorMessage);
      throw new Error(errorMessage);
  ...
  const getCategoriesWithEventCount = useCallback(() => {
  ...
  useEffect(() => {
    ...
    const subscription = eventService.categories.subscribe((payload) => {
